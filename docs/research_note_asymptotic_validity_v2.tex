...
\begin{enumerate}[leftmargin=*]
    ...
    \end{equation}
    where $\mathcal{V}_R(k) \in \{0, 1\}$ indicates whether candidate $k$ satisfies all three runtime asymptotic validity diagnostics (variance accumulation, jump dispersion, and proposal-adjusted binomial consistency). At the anchor $i=1$ ($\tau_{l, 1} = 0$), all items are routed to automated acceptance or oracle review, ensuring $\text{Recall} \equiv 1$ deterministically, providing a guaranteed valid baseline ($\mathcal{V}_R(1) \equiv 1$). By stopping at the first non-rejection, Fixed Sequence Testing guarantees family-wise error rate control $\le \delta_R$ across all $K_l$ lower threshold candidates without Bonferroni penalties.
...
\begin{enumerate}[leftmargin=*]
    ...
    \end{itemize}
    Crucially, under-benchmarking by Diagnostic~1 is \textbf{actively compensated by Diagnostic~3}: stopping is impossible unless $n \ge n_{\text{binom}}(k; p_0, \delta) \ge n_{\text{binom}}(0) \approx \frac{\log(1/\delta)}{p_0}$ (typically $70\text{--}90$ samples), blocking premature stopping even if Diagnostic~1 is satisfied on sample $t=2$. Conversely, potential overestimation of process variance by uncalibrated proxies is strictly clamped by the exact boundary null caps and theoretical ceiling ($v_{0, \text{target}} \le n_0 (1 - \gamma) w_{\max}$), ensuring Diagnostic~1 does not permanently block stopping on valid thresholds.
